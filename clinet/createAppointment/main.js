...
const getHour = async () => {
  ...
  try {
    const therapistDetailsUrl = `http://localhost:8080/api/createNewAppointment/${getDate}/${therapistID}`;
    ...
    buttonSubmit.onclick = () => {
      if (selectedHour) {
        ...
        const newAppointment = {
          ...
          is_available: false,
          time: selectedHour,
          therapist_name: therapistID,
          patient_name: data._id,
...
function showPopup() {
  ...
  setTimeout(() => {
    window.location.href = '../privateArea/private-area.html'; // Redirect to private area URL
