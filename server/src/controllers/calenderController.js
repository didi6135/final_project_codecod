...
router.get('/createNewAppointment/:date/:therapist_id', async (req, res) => {
    try {
       const dateToCheck = req.params.date
       const therapistName = req.params.therapist_id
...
router.delete('/deleteAppointment/:appointment_id', async (req, res) => {
    try {
       const appointment_id = req.params.appointment_id
        const deleteOne = await Appointment.findOneAndDelete({_id: appointment_id})
